...
S.R = .01*diag([.5]);%no distance

% initial mean and covariance
xt = [0; 0; 0; 1]; % true state

P = 5*.01*diag([1 1 1 4]) % covariance
...
for k=1:N,
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%
  z = fangle(S.h(xts(:,k+1), S) + sqrt(S.R)*randn(S.r,1));  % generate measurement
